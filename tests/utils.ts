...
const VITE_PORT = process.env.VITE_PORT || 6969;
...
4`);
    ...
    await expect(content).toHaveText(
      `1console.log('hello world!')console.log(\"Hello, Lorem Post 3!\")`
    );
  }
};
...
export const examineLoremPostEditor = async (page: Page, index: number) => {
  ...
  if (index === 0) {
    ...
    await expect(editor).toHaveText(
      `1234fn main() {    println!("Hello, world!");}`
    );
  }
};

export const expectIcon = async (
  page: Page,
  iconType: "upvote" | "downvote",
  state: "filled" | "empty",
  timeout = 1500
) => {
  await expect(
    iconType === "upvote"
      ? page.getByTestId("upvote-icon")
      : page.getByTestId("downvote-icon")
  ).toHaveAttribute("fill", state === "filled" ? "currentColor" : "", {
    timeout,
  });
};
