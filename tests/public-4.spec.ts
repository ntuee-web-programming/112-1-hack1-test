...
await fetch("http://localhost:8000/api/init", { method: "POST" });
...
test("4.3 Render User Information - profile picture 1 (6%)", async ({
  page,
}) => {
  await login(page);
  await page.getByTestId("tab-settings").click({ timeout: 1200 });
...
test("4.5 Render User Information - profile picture 2 (5%)", async ({
  page,
}) => {
  await login(page, "new test");
  await page.getByTestId("tab-settings").click({ timeout: 1200 });
